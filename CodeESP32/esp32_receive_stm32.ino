...
bool isWifiReallyConnected() {
  HTTPClient http;
  http.setTimeout(500); // Timeout ngắn
  http.begin("http://ldciot.id.vn/projects/dbiot/receive.php");  // Trang nhỏ nhẹ, tạo riêng nếu được
...
void loop() {
  ...
  wifiIsOK = isWifiReallyConnected();
   if(!wifiIsOK && ssid !="" && wifiMode !=0) {
    ESP.restart();
